...
$component = Components::list()['settings'];

// на случай если кто то захочет изменить в админке (это не то что плохо - просто не надо)
if($component['max_compare_level'])
	throw new Error('Компонент settings не должен иметь в настройках предела уровня анализа уникальных данных для Рассылки (рассыки только изменившихся значений)');
...
$settings = array();
foreach($component['default'] as $key=>&$setting)
{
